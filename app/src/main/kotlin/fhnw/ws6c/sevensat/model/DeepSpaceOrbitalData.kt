...
class DeepSpaceOrbitalData(
  ...
  incl: Double = 0.0,
  val raan: Double = 0.0,
  val argper: Double = 0.0,
  val meanan: Double = 0.0,
  val catnum: Int = 0,
...
) : OrbitalData(name, epoch, meanmo, eccn, incl) {
  ...
  init {
    ...
    c4 =
      2 * dsv.xnodp * coef1 * dsv.aodp * dsv.betao2 * (eta * (2.0 + 0.5 * etasq) + eccn
          * (0.5 + 2 * etasq) - 2 * CK2 * tsi / (dsv.aodp * psisq)
          * (-3 * x3thm1 * (1.0 - 2 * eeta + etasq * (1.5 - 0.5 * eeta)) + (0.75 * x1mth2
          * (2.0 * etasq - eeta * (1.0 + etasq)) * cos(2.0 * omegao))))
  ...
  internal fun calculateSDP4(tSince: Double) {
    synchronized(this) {
      ...
      val xmdf = xmo + dsv.xmdot * tSince
      val tsq = tSince * tSince
      val templ = t2cof * tsq
      dsv.xll = xmdf + dsv.xnodp * templ
      dsv.omgadf = omegao + dsv.omgdot * tSince
      val xnoddf = xnodeo + dsv.xnodot * tSince
      dsv.xnode = xnoddf + xnodcf * tsq
      val tempa = 1.0 - c1 * tSince
      val tempe = bstar * c4 * tSince
      dsv.xn = dsv.xnodp
      dsv.t = tSince
      deep.dpsec()
  ...
  inner class DeepSpaceCalculator(private val dsv: DeepSpaceValueObject) {
    ...
    fun dpsec() {
      ...
      dsv.em = eccn + sse * dsv.t
      dsv.xinc = xincl + ssi * dsv.t
      if (dsv.xinc < 0) {
        dsv.xinc = -dsv.xinc
